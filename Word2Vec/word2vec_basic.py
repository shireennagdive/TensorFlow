...
def generate_batch(data, batch_size, num_skips, skip_window):
    ...
    for i in range(batch_size // num_skips):
        ...
        for j in range(num_skips):
            batch[i * num_skips + j] = window[skip_window]  # Try to think of an alternative
            labels[i * num_skips + j, 0] = window[label_words[l]]
            l += 1
    ...
    for step in xrange(max_num_steps):
        ...
        if step % checkpoint_step == 0:
            ...
            for i in xrange(valid_size):
                ...
                # top_k = 8  # number of nearest neighbors # configuration for {first,american,would}
                top_k = 20
...
if __name__ == '__main__':
    ...
    num_skips = 8  # How many times to reuse an input to generate a label. #samples_in_window
    ...
    # configuration for {first,american,would}
    # valid_size = 3
    # valid_examples = list()
    # valid_examples.append(dictionary['first'])
    # valid_examples.append(dictionary['american'])
    # valid_examples.append(dictionary['would'])
